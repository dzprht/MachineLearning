...
class ObservationEncoder:
    ...
    def encode(self, obs: dict, executor_state=None) -> np.ndarray:
        ...
        values.extend(self._market_features(obs.get("market", {})))
        values.extend(self._shop_features(obs.get("town", {}), turns, self.configuration))
    ...
    @staticmethod
    def _market_features(market: dict) -> list[float]:
        """Price/inventory-deviation block. Reused as-is by other v1 encoders."""
    ...
    @staticmethod
    def _shop_features(town: dict, turns: int, configuration) -> list[float]:
        """Shop-count/daily-demand block. Reused as-is by other v1 encoders."""
        unlocked_shops = town.get("unlocked_shops", [])
        shop_counts = {shop: unlocked_shops.count(shop) for shop in SHOP_ORDER}
        values = [_unit(shop_counts[shop], MAX_SHOP_INSTANCES) for shop in SHOP_ORDER]
