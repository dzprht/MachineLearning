...
class NumpyActor:
    ...
    @classmethod
    def load(cls, path: Path | str) -> "NumpyActor":
    ...
    def predict(self, observation: np.ndarray, action_mask: np.ndarray) -> int:
        ...
        if mask.shape != (len(ACTION_NAMES),) or not mask.any():
            raise ValueError("action_mask must enable at least one of the six actions")
        logits = self.logits(observation)
        return int(np.argmax(np.where(mask, logits, -np.inf)))


def export_actor(model, weights_path: Path | str, schema: dict, schema_path: Path | str) -> None:
